...
本章目标：理解 Eino 的 Component 抽象，用最小代码调用一次 ChatModel（支持流式输出），并掌握 `schema.Message` / `schema.AgenticMessage` 的运行时切换方式。
...
本示例代码实际使用的是泛型接口 `model.BaseModel[M]`。当 `MESSAGE_KIND=message` 时，`M` 是 `*schema.Message`；当 `MESSAGE_KIND=agentic` 时，`M` 是 `*schema.AgenticMessage`。
...
1. **选择消息类型**：根据 `MESSAGE_KIND` 选择 `*schema.Message` 或 `*schema.AgenticMessage`
2. **创建 ChatModel**：根据消息类型和 `MODEL_TYPE` 环境变量选择普通 model 或 agentic model
3. **构造输入 messages**：通过 `msgops.NewSystem[M]` / `msgops.NewUser[M]` 创建对应消息类型
4. **调用 Stream**：使用 `model.BaseModel[M].Stream()`，返回 `StreamReader[M]`
5. **打印结果**：迭代 `StreamReader` 逐帧打印 assistant 回复
