...
type PlanItem struct {
	ActivityType         ActivityType `json:"activity_type" jsonschema:"description:活动类型,enum:attraction,enum:performance,enum:restaurant,enum:move"`
	StartTime            string       `json:"start_time" jsonschema:"description:活动开始时间，格式为15:04"`
	PerformanceStartTime *string      `json:"performance_start_time" jsonschema:"description:如果为 performance 类型，是演出开始时间，格式为15:04. 如果为其他类型，忽略这个参数或传 NULL"`
	PerformanceDuration  *int         `json:"performance_duration" jsonschema:"description:如果为 performance 类型，是演出时长，单位为分钟，如果为其他类型，忽略这个参数或传 Null"`
...
func ValidatePlanItems(_ context.Context, in *ValidatePlanItemsRequest) (out *ValidatePlanItemsResponse, err error) {
	...
	for i, item := range in.PlanItems {
		...
		case "移动":
			item.ActivityType = ActivityTypeMove
		default:
		}

		// 1. validate basic information is correct by query activity name, location, activityType, performance duration
		if item.ActivityType != ActivityTypeAttraction && item.ActivityType != ActivityTypePerformance && item.ActivityType != ActivityTypeRestaurant && item.ActivityType != ActivityTypeMove {
		...
		if item.ActivityType == ActivityTypePerformance {
			if item.PerformanceDuration == nil || *item.PerformanceDuration <= 0 {
				result.IsValid = false
		...
		// 3. validate PerformanceStartTime + PerformanceDuration is at least 10 minutes before the StartTime of next PlanItem, if there is a next PlanItem
		if item.ActivityType == ActivityTypePerformance && i < len(in.PlanItems)-1 {
			...
			if err != nil {
				...
				endTime := performanceStartTime.Add(time.Duration(*item.PerformanceDuration) * time.Minute)
		...
		// 4. if it's an attraction, StartTime + QueueTime + 10 minutes should be before the StartTime of next PlanItem, if there is a next PlanItem
		if item.ActivityType == ActivityTypeAttraction && i < len(in.PlanItems)-1 {
			...
			if err != nil {
				...
				endTime := startTime.Add(time.Duration(*item.QueueTime) * time.Minute).Add(10 * time.Minute)
				nextStartTime, err := time.Parse("15:04", in.PlanItems[i+1].StartTime)
				if err != nil {
					result.IsValid = false
					result.ErrMsg = fmt.Sprintf("Invalid start time format for next plan item %s", in.PlanItems[i+1].ActivityName)
		...
		if i > 0 {
			...
			case ActivityTypePerformance:
				...
				prevEndTime = prevStartTime.Add(time.Duration(*prevItem.PerformanceDuration) * time.Minute)
