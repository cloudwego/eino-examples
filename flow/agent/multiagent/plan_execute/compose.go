...
type state struct {
	messages            []*schema.Message // 给 Executor 的上下文
	messagesForDeepseek []*schema.Message // 给 DeepSeek 的上下文，对涉及 ToolCall 的消息做了转化
}
...
func NewMultiAgent(ctx context.Context, config *Config) (*PlanExecuteMultiAgent, error) {
	...
	modelPreHandle := func(systemPrompt string, isDeepSeek bool) compose.StatePreHandler[[]*schema.Message, *state] {
		return func(ctx context.Context, input []*schema.Message, state *state) ([]*schema.Message, error) {
			for _, msg := range input {
				...
				if msg.Role == schema.Tool { // 有 DeepSeek 服务商如火山引擎，目前不支持传入 ToolMessage
					state.messagesForDeepseek = append(state.messagesForDeepseek, schema.AssistantMessage(msg.Content, nil))
				} else {
					state.messagesForDeepseek = append(state.messagesForDeepseek, msg)
				}
			}

			if isDeepSeek {
				return append([]*schema.Message{schema.SystemMessage(systemPrompt)}, state.messagesForDeepseek...), nil
			}
	...
	_ = graph.AddToolsNode(nodeKeyTools, toolsNode, compose.WithStatePreHandler(func(ctx context.Context, in *schema.Message, state *state) (*schema.Message, error) {
		...
		for _, toolCall := range in.ToolCalls { // 有 DeepSeek 服务商如火山引擎，目前不支持传入带 ToolCall 信息的消息
			state.messagesForDeepseek = append(state.messagesForDeepseek, schema.AssistantMessage(fmt.Sprintf("call %s with %s, got response:", toolCall.Function.Name, toolCall.Function.Arguments), nil))
			logs.Infof("tool call name= %s, id= %s", toolCall.Function.Name, toolCall.ID)
		}
		return in, nil
	}))
