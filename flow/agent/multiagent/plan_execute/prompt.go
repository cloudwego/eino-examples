...
const (
	defaultPlannerPrompt = `你会收到用户关于乐园 A 的游园规划的问题。你的工作是仔细思考如何回答这些问题，需要获取和分析哪些信息，形成一个回答问题的思路，把这个思路输出出去，交给后面实际的问题解决者去解决。注意，你不是要给出这个问题的答案，而是要给出这个问题的解决思路和解决计划。

比如，用户的问题是”乐园 A 中有哪些刺激的游乐设施？“，你的输出应当是是”如何拆解这个问题，如何一步步的获取信息，解决子问题，才能最终回答好这个问题“，比如：

1. 先去查询乐园 A 中有哪些游乐设施
2. 再去看这些游乐设施的具体信息
3. 最后再筛选出有哪些刺激的游乐设施

在你拆分的各步骤中能调用的 API 包括：
...
- 同时，每次收到一版一日完整日常安排后或优化后的方案时（包含游乐设施+表演+用餐+移动），需要调用 validate_plan_items 工具来验证整个计划是否自洽，无论验证结果是否自洽，都输出""
- 如果你收到的完整日常安排仅仅是 arrange_performances 的返回结果，而不包含游乐设施、餐厅等其他计划项，务必**不要**调用 validate_plan_items.

注意，你的工作只是调用工具，切记**不要**规划真正的行程安排。当所有的工具都调用完毕后，直接返回""`
...
8. 虽然表演不需要排队，但是热门的表演还是需要适当提前排队，为了找到合适的位置。
9. 需要注意一个游乐设施的真实体验时间，一般需要花 10 分钟左右。所有一个游乐设施需要的时间 = 排队时间 + 10 分钟。如果买高速票，可认为固定在 10 分钟。
10. 游乐设施的排队时间在早上刚开园半小时和晚上压轴表演之前半小时一般会大幅减少 50% 左右。午间排队时间会减少 20% 左右。
11. 如果在排队低峰期使用高速票，要慎重评估性价比，因为低峰期的排队时间会大幅减少。
12. 如果午饭或者晚饭选择了快餐，就不需要再吃对应的正餐，反过来一样。
13. 占座、排队等不要单独列成一个计划项，而应当与对应的项目合并在一起计算时间。
...
| 开始时间        |表演开始时间|表演时长| 游乐设施、表演或餐厅的准确名称                                                                 |预计排队时间| 区域         |类型|费用单价|总费用|备注                     |
|---------------|---|--------------------------------------------------------------------------|--------------|---|----|--|--|--|------------------------------|
| 时间 1   |14:00|表演实际市场|表演的准确名称                                  |0| 地点 1  |表演|0|0 备注 1               |
| 时间 2   |不是表演，留空 |不是表演，留空|游乐设施的准确名称                                                  |30分钟| 地点 2     |游乐设施|高速票 100|100*2=200| 备注 2                    |
| 时间 3   |不是表演，留空 |不是表演，留空|餐厅的准确名称                                                      |0| 地点 3     |餐厅|0|0| 备注 3                   |
