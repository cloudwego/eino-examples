...
// NewMermaidGenerator creates a generator suited for Graph/Chain diagrams.
// It renders edges without additional labels and uses standard Mermaid arrows.
func NewMermaidGenerator(w io.Writer) *MermaidGenerator {
	return &MermaidGenerator{w: w}
}

// NewMermaidAuto creates a generator that auto-writes Markdown and attempts PNG/SVG generation.
// If dir is empty, current working directory is used. File name is derived from graph name or defaults to "topology".
func NewMermaidAuto(dir string) *MermaidGenerator {
	return &MermaidGenerator{autoWrite: true, outDir: dir, makeImages: true}
}

// NewMermaidAutoWorkflow creates a workflow-style auto generator.
func NewMermaidAutoWorkflow(dir string) *MermaidGenerator {
	return &MermaidGenerator{autoWrite: true, outDir: dir, workflowStyle: true, makeImages: true}
}
...
func (m *MermaidGenerator) generate(info *compose.GraphInfo) {
	...
	m.renderGraph(sb, info, "", 1, m.workflowStyle)
