...
This example demonstrates a sophisticated "human-in-the-loop" pattern: **Review and Edit**. It showcases a multi-agent workflow where one agent generates content, another agent requests human review, and the system supports iterative refinement based on human feedback.
...
2. **Review Agent**: An agent that acts as a bridge between the Writer Agent and the human reviewer. It:
   ...
   - Triggers an interrupt to request human review
...
3. **Review Interrupt**: The Review Agent captures the content and triggers an interrupt, presenting it to the human for review
4. **Human Review**: The human can either:
...
6. **Loop Continuation**: The process can repeat for multiple iterations of review and refinement
...
2. A prompt asking for your review feedback
...
graph TD
    ...
    D --> E[Interrupt: Request Review] --> F{Human Review};
...
Here's a real trace from running the example, showing how the iterative review process works:
...
### State Management
- The `ReviewEditInfo` struct maintains:
  - `OriginalContent`: The content being reviewed
  - `ReviewComment`: Human feedback for improvements
